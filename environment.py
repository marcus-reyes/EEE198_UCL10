...
class PruningEnv:
    ...
    Attributes:
        ...
        validation_dl (data.DataLoader): data subset for evaluation in
            Simulated Annealing 
    ...
    def __init__(self, dataset='cifar10', model_type='basic'):
        ...
        self.train_dl, self.test_dl, self.validation_dl = self.get_dataloaders()
    ...
    def get_dataloaders(self):
        ...
        Returns:
            train_dl, test_dl, validation_dl: dataloaders of datasets
    ...
    def forward_pass(self, num_of_batches):
        ...
        with torch.no_grad():
            for _ in range(num_of_batches):
                data, target = next(iter(self.validation_dl))
        ...
        return 100. * correct / (num_of_batches*self.validation_dl.batch_size)            

    def param_to_mask(self, ratio, method):
        """ MARCUS: concise definition 
        ...
        if method == 'weight_norm':
            ...
            mag_rank = torch.topk(pooled_weights_mean,
                                  int(num_filters*ratio),
                                  largest = False)
            mask[mag_rank[1]] = 0
            ...
            # Variance across the channels, and not the individual kernel
            # elements
            ...
            for idx, train_data in enumerate(self.train_dl):
                    ...
                    for key, var in self.model.named_parameters():
                        if key == self.layer + '.weight':
                            pooled_grad = torch.squeeze(F.avg_pool2d(
                                                            torch.abs(var.grad),
                                                            var.grad.size()[-1]))
